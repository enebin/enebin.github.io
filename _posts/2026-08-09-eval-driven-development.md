...
핵심 주장은 한 줄이다. **평가(eval)를 나중에 하는 검사가 아니라, 개발의 출발점으로 두라는 것.** 지표와 배포 기준을 먼저 정하고, 실제로 관찰한 실패에서 평가자를 만들고, 그 기준을 통과해야 내보낸다. 팀은 이걸 Eval 주도 개발(Eval-Driven Development, EDD)이라고 부른다.

이름은 테스트 주도 개발(TDD)에서 따온 티가 난다. 테스트를 먼저 쓰고 코드를 짜듯, 평가를 먼저 만들고 기능을 붙인다. 다만 뒤에서 보겠지만 실제로 하는 일은 TDD와 꽤 다르다.
...
**루브릭을 모호함 없이 쓴다.** "답변이 자연스러운가, 1~5점"은 루브릭이 아니다. 심판마다, 실행마다 다르게 읽는다. 글에 나온 예시는 아니지만 내가 이해한 대로 옮겨보면 이런 차이다.

> **5점** — 질문에 직접 답하고, 근거로 든 사실이 전부 제공된 문서 안에 있다.
>
> **3점** — 질문에 답하지만, 문서에 없는 사실이 한 개 이하로 섞여 있다.
>
> **1점** — 질문에 답하지 않거나, 문서에 없는 사실이 두 개 이상이다.

각 점수가 무엇인지 문장으로 정의되어 있으면 판정이 크게 흔들리지 않는다. 점수와 함께 판단 근거를 쓰게 하면 나중에 왜 그렇게 채점했는지 확인할 수도 있다.
...
**4단계, 확장과 관찰.** 검증된 심판을 수천 건으로 확장하고, 운영에서 계속 돌린다.

4단계는 끝이 아니다. 운영에서 돌리다 보면 처음에 못 본 실패 유형이 나온다. 그러면 다시 1단계로 돌아간다. 평가 체계는 한 번 만들고 끝나는 산출물이 아니라 계속 도는 루프다.

시작할 때 필요한 라벨은 50~100개 정도면 된다. 처음부터 큰 데이터셋을 모으려다 아무것도 시작 못 하는 쪽이 훨씬 흔하다.
